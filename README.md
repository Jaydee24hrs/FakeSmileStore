...
higher-res, opaque re-export of the brick "smile" mark supplied specifically
for icon use (NOT the same file as `images/Fakesmile-1.webp`, the header/
footer logo, which stays untouched and transparent). The full logo's "FAKE
SMILE" text banner turns to mush at 16-32px, so the script crops just the
bottom smile arc, flood-fills away `mainfav.webp`'s white background (from
the FULL image's corners, before cropping — flood-filling from the crop's
own corners breaches the ring where the crop boundary crosses a JPEG-softened
transition), and outputs: `favicon.ico` (16-256px multi-size, what Google
...
To use a new logo: replace `images/mainfav.webp`, re-check the crop
coordinates (`CROP_BOX` in the script — they're specific to that file's
layout), run `python tools/make-favicons.py` (needs Pillow), done — no HTML
changes needed, every page already links the fixed output paths. A changed
favicon can take days to refresh in Google's cached search-result icon — no
code fixes that faster.
