...
**Favicon.** Every page carries a full icon set generated by
`tools/make-favicons.py` from `images/mainfav.webp` — a dedicated,
higher-res, opaque re-export of the **complete** logo supplied specifically
for icon use (NOT the same file as `images/Fakesmile-1.webp`, the header/
footer logo, which stays untouched and transparent). The script flood-fills
away `mainfav.webp`'s white background (from the full image's corners — the
outer ring there is a solid, unbroken circle, so a generous threshold can't
leak into the artwork) and trims/re-pads the transparent margin so the
complete circular badge (crest + "FAKE SMILE" banner + smile arc) fills the
frame, then outputs: `favicon.ico` (16-256px multi-size, what Google
Search/old browsers auto-request), `images/favicon-{16,32,48}x{16,32,48}.png`,
...
Note: the "FAKE SMILE" text isn't legible at 16px (no browser tab renders
text that small off *any* logo this detailed) — the brand owner chose the
complete logo over a simplified/cropped mark anyway; see git history if a
tinier-scale-optimized version is wanted later. To use a new logo: replace
`images/mainfav.webp`, run `python tools/make-favicons.py` (needs Pillow),
done — no HTML changes needed, every page already links the fixed output
paths. A changed favicon can take days to refresh in Google's cached
search-result icon — no code fixes that faster.
...
- **Favicon: use the complete logo, not the cropped smile arc.** Brand-owner
  ...
  `tools/make-favicons.py` no longer crops to the bottom smile arc — it
  flood-fills `mainfav.webp`'s background away, trims the transparent
  margin, and pads the *whole* circular badge (crest + "FAKE SMILE" banner +
  smile) to fill the favicon frame. Regenerated `favicon.ico` (all 6 sizes
  re-verified loadable) and every PNG variant; no HTML changes.
- **Favicon source swapped to a dedicated master (`images/mainfav.webp`).**
  The user supplied a cleaner, higher-res re-export of the logo specifically
  for icon use. `tools/make-favicons.py` now flood-fills away its white
  background (from the full image's corners, pre-crop — doing it after
  cropping breached the ring at a JPEG-softened seam) before cropping the
  same bottom-smile-arc region and regenerating every icon file. Converted
  the original upload (`mainfav.jpg`) to WebP per the site's image
  convention and removed the `.jpg`. No HTML changed — all icon paths are
  the same fixed root-absolute URLs.
